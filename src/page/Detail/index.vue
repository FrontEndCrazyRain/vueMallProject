            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <a
                  href="javascript:"
                  class="mins"
                  @click="shoppingNum = --shoppingNum < 0 ? 0 : shoppingNum">
                  -
                </a>
              </div>
              <div class="add">
                <a @click="saveOrUpdate">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    changeActive(item, valueList) {
      valueList.forEach(value => {
        value.isChecked = 0;
        if (value.saleAttrValueName === item.saleAttrValueName) {
          item.isChecked = 1;
        }
      });
    },
    ...
    changeNum({ target }) {
      this.shoppingNum = Math.floor(target.value);
    },
    async saveOrUpdate() {
      let num = this.shoppingNum;
      let skuId = this.$route.params.skuId;
      let res = await api.saveOrUpdate({ skuId, num });
      res.code === 200
        ? sessionStorage.setItem('skuInfo', JSON.stringify(this.skuInfo)) &
          this.$router.push({ name: 'AddCartSuccess', query: { num } })
        : console.log(res.message);
    },
  },
  mounted() {
    console.log('this.categoryView', this.categoryView);
  },
};
</script>
